...
class PgKnexCrudStrategy extends PgCrudStrategy {
   ...
   */
    ...
    getCountForPrj (table, projectId) {
        let knex = this.knex;
        return knex.raw(
            'select count("' + table + '"."id") as "value", "data_type"."name" as "label" ' +
            'from "' + table + '" ' +
            (table === 'data_type' ? '' : 'inner join data_type on data_type."id" = "' + table + '"."type" ') +
            'inner join "super_type" on "super_type"."id" = "data_type".super_type ' +
            'where "super_type".skip_paging = false ' +
            (projectId ? 'and "data_type"."project" = ' + projectId : '') +
            'group by "label" ' +
            'union ' +
            'select distinct -1 as "value", "data_type"."name" as "label" ' +
            'from "data_type" ' +
            'inner join "super_type" on "super_type"."id" = "data_type".super_type ' +
            'where "super_type".skip_paging = true ' +
            (table === 'data_type' ? '' : 'and "data_type"."model" = \'' + table.charAt(0).toUpperCase() + table.slice(1) + '\' ') +
            (projectId ? 'and "data_type"."project" = ' + projectId : '')
        );
    }
   ...
   */
    getCountsForDashboard (projectId) {
        ...
        // let queryDts = this.getCountForPrj('data_type', projectId);
        let querySamples = this.getCountForPrj('sample', projectId);
        let queryData = this.getCountForPrj('data', projectId);
        /*
        let knex = this.knex;
   ...
   */
    ...
    getInfoForBarChartDatediff (
        ...
        if (fromIsChild == 1) {
            let joinFromChild = joinPoolingTables.get(fromModel + "_" + toModel);
            query.innerJoin(joinFromChild.name + " as from_to", function () {
                this.on(fromMdAlias + ".id", "=", "from_to." + joinFromChild.childColumn)
                    .andOn(toMdAlias + ".id", "=", "from_to." + joinFromChild.parentColumn);
            });
        }

        // --------------JOIN: TO IS CHILD OF FROM
        if (toIsChild == 1) {
            let joinToChild = joinPoolingTables.get(toModel + "_" + fromModel);
            query.innerJoin(joinToChild.name + " as to_from", function () {
                this.on(toMdAlias + ".id", "=", "to_from." + joinToChild.childColumn)
                    .andOn(fromMdAlias + ".id", "=", "to_from." + joinToChild.parentColumn);
            });
        }
   ...
   */
    getDataTypesByRolePrivileges (criteria) {
        let knex = this.knex;
        let query = knex('data_type').distinct('data_type.id', 'data_type.name', 'super_type.schema', 'data_type.project', 'datatype_privileges.privilege_level').from('data_type');
        query.innerJoin('datatype_privileges', 'data_type.id', 'datatype_privileges.data_type');
        query.innerJoin('xtens_group', 'xtens_group.id', 'datatype_privileges.xtens_group');
        query.innerJoin('super_type', 'super_type.id', 'data_type.super_type');
        query.innerJoin('group_members__operator_groups', 'xtens_group.id', 'group_members__operator_groups.group_members');
        ...
        return knex.transaction(function (trx) {
            ...
            // NOTE: should I also update parent_subject, parent_sample and/or parent_data? Should it be proper/safe?
                ...
                .then(function (foundDataAssociations) {
                    return BluebirdPromise.map(data.parentData || [], function (idParent) {
                        // if the associations exists, leave it alone
                        if (_.find(foundDataAssociations, {
                            'data_childrenData': idParent
                        })) {
                ...
                .then(function (foundSampleAssociations) {
                    return BluebirdPromise.map(data.parentSample || [], function (idParent) {
                        // if the associations exists, leave it alone
                        if (_.find(foundSampleAssociations, {
                            'sample_childrenData': idParent
                        })) {
                ...
                .then(function (foundSubjectAssociations) {
                    return BluebirdPromise.map(data.parentSubject || [], function (idParent) {
                        // if the associations exists, leave it alone
                        if (_.find(foundSubjectAssociations, {
                            'subject_childrenData': idParent
                        })) {
