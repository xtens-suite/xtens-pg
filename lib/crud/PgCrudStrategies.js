...
class PgKnexCrudStrategy extends PgCrudStrategy {
    ...
    findData(criteria) {
        ...
        switch (criteria.model) {
            case "Data":
                query = knex.distinct('data.id', 'data.type', 'data.metadata', 'data_type.project', 'data_type.name', 'datatype_privileges.privilege_level', 'data.notes', 'data.tags', 'data.parent_subject', 'data.parent_data', 'data.parent_sample', 'data.acquisition_date as date', 'data.created_at', 'data.updated_at').select().from('data');
    ...
    getDataTypesByRolePrivileges(criteria) {
        ...
        let knex = this.knex;
        let query = knex.select('data_type.id', 'data_type.name', 'data_type.schema', 'data_type.project', 'datatype_privileges.privilege_level').from('data_type');
        query.innerJoin('datatype_privileges', 'data_type.id', 'datatype_privileges.data_type');
        query.innerJoin('xtens_group', 'xtens_group.id', 'datatype_privileges.xtens_group');
        query.innerJoin('group_members__operator_groups', 'xtens_group.id', 'group_members__operator_groups.group_members');
        query.innerJoin('operator', 'operator.id', 'group_members__operator_groups.operator_groups');
