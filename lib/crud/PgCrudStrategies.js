...
class PgKnexCrudStrategy extends PgCrudStrategy {
    ...
    findData (criteria) {
        ...
        switch (criteria.model) {
            ...
            case "Sample":
                query = knex.select('sample.id', 'sample.type', 'sample.metadata', 'data_type.project', 'data_type.name', 'subject.code', 'datatype_privileges.privilege_level', 'sample.notes', 'sample.tags', 'sample.biobank', 'biobank.acronym', 'sample.biobank_code', 'sample.created_at', 'sample.updated_at').from('sample');
    ...
    createData (data, dataTypeName) {
        let knex = this.knex; let that = this;
    ...
    updateData (data, dataTypeName) {
        let that = this; let knex = this.knex; let updatedData;
        let partitionedFiles = _.partition(data.files, file => {
            return !file.id;
        });
        let existingFiles = partitionedFiles[1]; let notExistingFiles = partitionedFiles[0];
        /*
    ...
    createSample (sample, sampleTypeName, project) {
        let that = this; let knex = this.knex;
        let fileSystemManager = this.fileSystemManager;
        ...
        if (!sample.parentSample) {
            ...
            return queryParentPrefix.then((res) => {
                let query = knex.first('sample.id', 'biobank_code', 'data_type.biobank_prefix', 'data_type.parent_code', 'data_type.parent_no_prefix').from('data_type').leftOuterJoin(
                    ...
                    'sample_parentsample__sample_childrensample', function () {
    ...
    updateSample (sample, sampleTypeName) {
        let that = this; let knex = this.knex; let updatedSample;
    ...
    createSubject (subject, subjectTypeName) {
        ...
        let createdSubject = null; let createdPersonalDetails;
    ...
    updateSubject (subject) {
        let knex = this.knex; let updatedSubject; let updatedPersonalDetails;
        let idProjects = _.cloneDeep(subject.projects) || [];
        delete subject.projects;
