...
class PgKnexCrudStrategy extends PgCrudStrategy {
        ...
        return knex.transaction(function(trx) {
          ...
          .then(function(rows) {
              ...
              return knex.returning('*').insert({
                  ...
                  'parent_no_prefix': dataType.ifParentNoPrefix ? true : false,
    ...
    createSample(sample, sampleTypeName, project) {
        ...
        return knex.transaction(function(trx) {
            ...
            return that.BiobankCodeHandler(sample, project, trx)
    ...
    BiobankCodeHandler(sample, project, trx) {
      if (sample.biobankCode) {
        return sample.biobankCode
      }
      let knex = this.knex;
      let query = knex.first('sample.id', 'biobank_code','data_type.biobank_prefix', 'data_type.parent_code', 'data_type.parent_no_prefix').from('sample').innerJoin('data_type', 'sample.type', 'data_type.id')
      .where('data_type.id', '=', sample.type).andWhere('biobank', '=', sample.biobank).andWhere('data_type.project', project)
      .orderBy('id', 'desc').transacting(trx);

      return query.then((result) => {
        console.log('PgKnexCrudStrategy.createSample - BiobankCodeHandler: ' + result);
        let nextCode;
        if (result.parent_code && !result.parent_no_prefix) {
          nextCode = result.biobank_prefix + sample.parentSample;
          return nextCode;
        }
        if (result.parent_no_prefix) {
          query = query.whereNull('parentSubject');
          return query.then((result) => {
            let lastBiobankCode = result ? result.biobank_code : START_SAMPLE_CODE;
