...
class PgKnexCrudStrategy extends PgCrudStrategy {
    ...
    getDataTypesByRolePrivileges(criteria) {
        ...
        let knex = this.knex;
        let query = knex.select('data_type.id', 'data_type.name', 'data_typedata_type.schema').from('data_type');
        ...
        if (criteria.idDataTypes) {
            let idDataTypes = criteria.idDataTypes.split(',').map(val => _.parseInt(val));
