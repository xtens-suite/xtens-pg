...
class PgKnexCrudStrategy extends PgCrudStrategy {
      ...
      return knex.transaction(function(trx) {
          ...
          .then(function(rows) {
            ...
            console.log("KnexStrategy.createData - data instance created with ID: " + createdData.id);
              return BluebirdPromise.map(data.parentData || [], function(idParent) {
                  return knex.returning('id').insert({
                      'data_parentData': idParent,
                      'data_childrenData': createdData.id
          ...
          .then(function() {
              return BluebirdPromise.map(data.parentSample || [], function(idParent) {
                  return knex.returning('id').insert({
                      'data_parentSample': idParent,
                      'sample_childrenData': createdData.id
          ...
          .then(function() {
              return BluebirdPromise.map(data.parentSubject || [], function(idParent) {
                  return knex.returning('id').insert({
                      'data_parentSubject': idParent,
                      'subject_childrenData': createdData.id
    ...
    updateData(data, dataTypeName) {
        ...
        return knex.transaction(function(trx) {
            ...
            .then(function(rows) {
                ...
                return knex('data_childrendata__data_parentdata').where('data_childrenData', data.id).transacting(trx);
            })
            .then(function(foundDataAssociations) {
                ...
                return BluebirdPromise.map(data.parentData || [], function(idParent) {
                    ...
                    // if the associations exists, leave it alone
                    if (_.findWhere(foundDataAssociations, {'data_parentSample': idParent})) {
                    ...
                    // otherwise insert a new association (no support for deleting associations is currently provided for consistency)
                      ...
                      return knex.returning('id').insert({
                          'data_parentData': idParent,
                          'data_childrenData': updatedData.id
                      }).into('data_childrendata__data_parentdata').transacting(trx);
                    }
            ...
            .then(function() {
                return knex('data_parentsample__sample_childrendata').where('sample_childrenData', data.id).transacting(trx);
            })
            .then(function(foundSampleAssociations) {

                return BluebirdPromise.map(data.parentSample || [], function(idParent) {
                    // if the associations exists, leave it alone
                    if (_.findWhere(foundSampleAssociations, {'data_parentSample': idParent})) {
                    ...
                    // otherwise insert a new association (no support for deleting associations is currently provided for consistency)
                      ...
                      return knex.returning('id').insert({
                          'data_parentSample': idParent,
                          'sample_childrenData': updatedData.id
                      }).into('data_parentsample__sample_childrendata').transacting(trx);
                    }
            ...
            .then(function() {
                return knex('data_parentsubject__subject_childrendata').where('subject_childrenData', data.id).transacting(trx);
            })
            .then(function(foundSubjectAssociations) {
                ...
                return BluebirdPromise.map(data.parentSubject || [], function(idParent) {
                    ...
                    // if the associations exists, leave it alone
                    if (_.findWhere(foundSubjectAssociations, {'data_parentSubject': idParent})) {
        ...
        return knex.transaction(function(trx) {
            ...
            .then(function(rows) {
              ...
              console.log("KnexStrategy.createSample - sample instance created with ID: " + createdSample.id);
                return BluebirdPromise.map(sample.parentSample || [], function(idParent) {
                    return knex.returning('id').insert({
                        'sample_parentSample': idParent,
                        'sample_childrenSample': createdSample.id
            ...
            .then(function() {
                return BluebirdPromise.map(sample.donor || [], function(idParent) {
                    return knex.returning('id').insert({
                        'sample_parentSubject': idParent,
                        'subject_childrenSample': createdSample.id
                    }).into('sample_parentsubject__subject_childrensample').transacting(trx);
                });
            })
            // store files on the FileSystem of choice (e.g. iRODS) in their final collection
            .then(function() {
    ...
    updateSample(sample, sampleTypeName) {
        ...
        return knex.transaction(function(trx) {
            ...
            .then(function() {
                return knex('sample_parentsample__sample_childrensample').where('sample_childrenSample', sample.id).transacting(trx);
            })
            .then(function(foundSampleAssociations) {

                return BluebirdPromise.map(sample.parentSample || [], function(idParent) {
                    // if the associations exists, leave it alone
                    if (_.findWhere(foundSampleAssociations, {'sample_parentSample': idParent})) {
                    ...
                    // otherwise insert a new association (no support for deleting associations is currently provided for consistency)
                      ...
                      return knex.returning('id').insert({
                          'sample_parentSample': idParent,
                          'sample_childrenSample': updatedData.id
                      }).into('sample_parentsample__sample_childrensample').transacting(trx);
                    }
            ...
            .then(function() {
                return knex('sample_parentsubject__subject_childrensample').where('subject_childrenSample', sample.id).transacting(trx);
            })
            .then(function(foundSubjectAssociations) {
                ...
                return BluebirdPromise.map(sample.donor || [], function(idParent) {
                    ...
                    // if the associations exists, leave it alone
                    if (_.findWhere(foundSubjectAssociations, {'sample_parentSubject': idParent})) {
                    ...
                    // otherwise insert a new association (no support for deleting associations is currently provided for consistency)
                      ...
                      return knex.returning('id').insert({
                          'sample_parentSubject': idParent,
                          'subject_childrenSample': createdData.id
                      }).into('sample_parentsubject__subject_childrensample').transacting(trx);
                    }
