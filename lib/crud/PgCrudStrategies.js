...
class PgKnexCrudStrategy extends PgCrudStrategy {
   ...
   */
    ...
    getInfoForBarChartDatediff (fromModel, fromDataTypeId, fromFieldName, toModel, toDataTypeId, toFieldName, period) {
        let knex = this.knex;
        let fromModelUF = fromModel;
        fromModel = fromModel.toLowerCase();

        let toModelUF = toModel;
        toModel = toModel.toLowerCase();

        let fromMdAlias = "from_" + fromModel;
        let fromDtAlias = "from_dt_" + fromModel;
        
        let toMdAlias = "to_" + toModel;
        let toDtAlias = "to_dt_" + toModel;
        
        let joinTable = toModel + "_parent" + fromModel + "__" + fromModel + "_children" + toModel;
	
        let fromField = "(" + fromMdAlias + ".metadata->'" + fromFieldName + "'->>'value')";
        let toField = "(" + toMdAlias + ".metadata->'" + toFieldName + "'->>'value')";
	
        let valueField = "date_part('day', " + toField + "::timestamp - " + fromField + "::timestamp)";
        let query = knex.select(knex.raw("count(" + valueField + ") as value"));
        query.from(fromModel + " as " + fromMdAlias);
        
        query.leftOuterJoin(joinTable, joinTable + "." + fromModel + "_children" + toModelUF, fromMdAlias + ".id");
        query.leftOuterJoin(toModel + " as " + toMdAlias, function() {
            this.on(toMdAlias + ".id", "=", joinTable + "." + toModel + "_parent" + fromModelUF)
