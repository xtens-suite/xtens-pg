...
class PgKnexCrudStrategy extends PgCrudStrategy {
   ...
   */
    ...
    getInfoForBarChartDatediff (fromModel, fromDataTypeId, fromFieldName, fromDataTypeByPt,
        toModel, toDataTypeId, toFieldName, toDataTypeByPt,
        period) {
        ...
        //--------------JOIN TABLE
        let joinInfo = null;
        if (fromModel == 'data' || toModel == 'data') {
            joinInfo = joinPoolingTables.get(toModel == 'data' ? (toModel + '_' + fromModel) : (fromModel + '_' + toModel));
        } else if (fromModel == 'sample' || toModel == 'sample') {
            joinInfo = joinPoolingTables.get(toModel == 'sample' ? (toModel + '_' + fromModel) : (fromModel + '_' + toModel));
        } else {
            joinInfo = joinPoolingTables.get('subject_subject');
        }

        //--------------SUBJECT TABLE
        let sbjTable = null;
        let joinSbjInfo = null;
        if (fromModel == 'subject') {
            sbjTable = fromMdAlias;
        } else if (toModel == 'subject') {
            sbjTable = toMdAlias;
        } else if (_.find(fromDataTypeByPt.parents, function (pt) {
            return pt.model == 'Subject';
        })) {
            sbjTable = "fromJoinSbj";
            joinSbjInfo = joinPoolingTables.get(fromModel + '_subject');
            isFromJoin = true; 
        } else if (_.find(toDataTypeByPt.parents, function (pt) {
            return pt.model == 'Subject';
        })) {
            sbjTable = "toJoinSbj";
            joinSbjInfo = joinPoolingTables.get(toModel + '_subject');
        }   

        //--------------QUERY
        let query = knex.select(fromField + " as fromDate");
        query = knex.select(toField + " as toDate");
        query = knex.select(sbjTable + ".code as subject_code");
        query.from(fromModel + " as " + fromMdAlias);
        
        query.leftOuterJoin(joinInfo.name + " as " + joinInfo.alias, joinInfo.alias + "." + joinInfo.childColumn, fromMdAlias + ".id");
        query.leftOuterJoin(toModel + " as " + toMdAlias, function() {
            this.on(toMdAlias + ".id", "=", joinInfo.parentColumn)
            .andOn(toMdAlias + ".type", parseInt(toDataTypeId))
        });

        if (joinSbjInfo) {
            query.innerJoin(joinSbjInfo.name + " as " + joinSbjInfo.alias, joinSbjInfo.alias + "." + joinSbjInfo.childColumn, 
            (sbjTable == "fromJoinSbj" ? fromMdAlias : toMdAlias) + ".id");
            query.innerJoin('subject as ' + sbjTable, sbjTable + '.id', model + '.type');
        }
        
        query.where(fromMdAlias + ".type", parseInt(fromDataTypeId));
        query.andWhereRaw(fromField + " is not null");
        query.groupBy("date");
        query.orderBy("date");
