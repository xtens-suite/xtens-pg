...
class PgKnexCrudStrategy extends PgCrudStrategy {
           ...
           return queryParentPrefix.then((res) => {
             ...
             return query.then((result) => {
               ...
               if (!isNaN(lastBiobankCode)) {
                  ...
                  return '' + (_.parseInt(lastBiobankCode) + 1);
               }
               else {
    ...
    getNextSubjectCode(subject, project, trx) {
        let subjCode;
        let prefix = SUBJ_CODE_PREFIX;
        let nextId = 1;
        if (subject.code) { // if a code is provided by the user use that
          return BluebirdPromise.resolve(subject.code);
        }
        if (!subject.type) {
          return BluebirdPromise.resolve(subjCode + nextId);
        }
        ...
        .then(function(result) {
            ...
            if (result.rows && result.rows.length > 0) {
              ...
              nextId = _.parseInt(splitted.slice(-1)) ? (_.parseInt(splitted.slice(-1))+1) : 1;
              subjCode = prefix ? prefix : SUBJ_CODE_PREFIX
