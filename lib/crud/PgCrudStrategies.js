...
class PgKnexCrudStrategy extends PgCrudStrategy {
    ...
    findData(criteria) {
        ...
        switch (criteria.model) {
            case "Data":
                query = knex.select('data.id', 'data.type', 'data.metadata', 'data_type.project', 'data_type.name as data_type_name', 'datatype_privileges.privilege_level', 'data.notes', 'data.tags', 'data.parent_subject', 'data.parent_data', 'data.parent_sample', 'data.acquisition_date as date', 'data.created_at', 'data.updated_at').from('data');
        ...
        return query.then(function(results) {
            return camelcaseKeys(results, {deep: false});
        });
          ...
          .then(function(rows) {
            ...
            console.log("KnexStrategy.createData - data instance created with ID: " + createdData.id
              return BluebirdPromise.map(data.parentData || [], function(idParent) {
                  return knex.returning('id').insert({
                      'data_parents': idParent,
                      'data_children': createdData.id
                  }).into('data_parentdata__data_datachildren').transacting(trx);
              });
          })
          .then(function() {
              return BluebirdPromise.map(data.parentSample || [], function(idParent) {
                  return knex.returning('id').insert({
                      'data_parents': idParent,
                      'sample_children': createdData.id
                  }).into('data_parentsample__sample_datachildren').transacting(trx);
              });
          })
          .then(function() {
              return BluebirdPromise.map(data.parentSubject || [], function(idParent) {
                  return knex.returning('id').insert({
                      'data_parents': idParent,
                      'subject_children': createdData.id
                  }).into('data_parentsubject__subject_datachildren').transacting(trx);
              });
          })
    ...
    updateData(data, dataTypeName) {
        ...
        return knex.transaction(function(trx) {
            ...
            .then(function(rows) {
                ...
                return knex('data_parentdata__data_datachildren').where('data_children', data.id).transacting(trx);
            })
            .then(function(foundDataAssociations) {
                ...
                return BluebirdPromise.map(data.parentData || [], function(idParent) {
                    ...
                    // if the associations exists, leave it alone
                    if (_.findWhere(foundDataAssociations, {'data_parents': idParent})) {
            ...
            .then(function() {
                return knex('data_parentsample__sample_datachildren').where('sample_children', data.id).transacting(trx);
            })
            .then(function(foundSampleAssociations) {

                return BluebirdPromise.map(data.parentSample || [], function(idParent) {
                    // if the associations exists, leave it alone
                    if (_.findWhere(foundSampleAssociations, {'data_parents': idParent})) {
                    ...
                    // otherwise insert a new association (no support for deleting associations is currently provided for consistency)
                      ...
                      return knex.returning('id').insert({
                          'data_parents': idParent,
                          'sample_children': updatedData.id
                      }).into('data_parentsample__sample_datachildren').transacting(trx);
                    }
            ...
            .then(function() {
                return knex('data_parentsubject__subject_datachildren').where('subject_children', data.id).transacting(trx);
            })
            .then(function(foundSubjectAssociations) {
                ...
                return BluebirdPromise.map(data.parentSubject || [], function(idParent) {
                    ...
                    // if the associations exists, leave it alone
                    if (_.findWhere(foundSubjectAssociations, {'data_parents': idParent})) {
    ...
    createSample(sample, sampleTypeName, project) {
        ...
        return knex.transaction(function(trx) {
            ...
            let query = knex.first('sample.id', 'biobank_code').from('sample').innerJoin('data_type', 'sample.type', 'data_type.id')
            .whereNull('parent_sample').andWhere('biobank', '=', sample.biobank).andWhere('data_type.project', project)
            ...
            .then(function(rows) {
              ...
              console.log("KnexStrategy.createSample - sample instance created with ID: " + createdSample.id
                return BluebirdPromise.map(sample.parentSample || [], function(idParent) {
                    return knex.returning('id').insert({
                        'sample_parents': idParent,
                        'sample_children': createdSample.id
                    }).into('sample_parentsample__sample_samplechildren').transacting(trx);
                });
            })
            .then(function() {
                return BluebirdPromise.map(sample.donor || [], function(idParent) {
                    return knex.returning('id').insert({
                        'sample_parents': idParent,
                        'subject_children': createdSample.id
                    }).into('sample_parentsubject__subject_samplechildren').transacting(trx);
                });
            })
            // store files on the FileSystem of choice (e.g. iRODS) in their final collection
            .then(function() {
    ...
    updateSample(sample, sampleTypeName) {
        ...
        return knex.transaction(function(trx) {
            ...
            .then(function() {
                return knex('sample_parentsample__sample_samplechildren').where('sample_children', sample.id).transacting(trx);
            })
            .then(function(foundSampleAssociations) {

                return BluebirdPromise.map(sample.parentSample || [], function(idParent) {
                    // if the associations exists, leave it alone
                    if (_.findWhere(foundSampleAssociations, {'sample_parents': idParent})) {
            ...
            .then(function() {
                return knex('sample_parentsubject__subject_samplechildren').where('subject_children', sample.id).transacting(trx);
            })
            .then(function(foundSubjectAssociations) {
                ...
                return BluebirdPromise.map(sample.donor || [], function(idParent) {
                    ...
                    // if the associations exists, leave it alone
                    if (_.findWhere(foundSubjectAssociations, {'sample_parents': idParent})) {
                    ...
                    // otherwise insert a new association (no support for deleting associations is currently provided for consistency)
                      ...
                      return knex.returning('id').insert({
                          'sample_parents': idParent,
                          'subject_children': createdData.id
                      }).into('sample_parentsubject__subject_datachildren').transacting(trx);
                    }
