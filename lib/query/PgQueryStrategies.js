...
class PgJSONQueryStrategy extends PgQueryStrategy {
    ...
    compose(criteria) {
        ...
        // ctes.push({
        //     alias: 'ow',
        //     commonTableExpression: 'ow AS (SELECT id FROM operator)',
        //     joinClause: 'LEFT JOIN ow ON ow.id = d.owner'
        // });
        // specificFields += "ow.id AS owner";

        ctes = ctes.concat(this.composeCommonTableExpression(query));
        console.log(ctes);
        let commonTableExpressions = "", joins = " ";
        query.select = "SELECT DISTINCT d.id, d.type, d.owner, " + specificFields + "d.metadata FROM " + query.table + " d";
