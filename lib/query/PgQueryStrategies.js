...
class PgJSONQueryStrategy extends PgQueryStrategy {
  ...
  */
    compose (criteria) {
        ...
        if (ctes.length) {
            // commonTableExpressions = "WITH " + _.map(ctes, 'commonTableExpression').join(", ");
            joins = " " + _.map(ctes, 'joinClause').join(" ") + " ";
        }
  ...
  */
    getSubqueryRowAttribute (element, previousOutput, tablePrefix) {
        ...
        if (element.fieldType === "boolean") {
            if (['IS NULL', 'IS NOT NULL'].indexOf(element.comparator) > -1) {
            ...
            subqueries = [];
            var formattedValues = "";
            if (['IS NULL', 'IS NOT NULL'].indexOf(element.comparator) < 0) {
                ...
                formattedValues = " (" + subqueries.join(",") + ")";
            }

            subquery = "(" + tablePrefix + "metadata->$" + (++previousOutput.lastPosition) + "->>'value')::" + element.fieldType.toLowerCase() +
            " " + element.comparator + formattedValues;
            previousOutput.parameters.push(element.fieldName);
        } else {
