...
class PgJSONQueryStrategy extends PgQueryStrategy {
    ...
    compose(criteria) {
        ...
        ctes.push({
            alias: 'ow',
            commonTableExpression: 'ow AS (SELECT id, first_name, last_name, email, address_information FROM operator)',
            joinClause: 'LEFT JOIN ow ON ow.id = d.owner'
        });
        specificFields += "ow.first_name AS owner_name, ow.last_name AS owner_surname, ow.email AS owner_email, ow.address_information AS owner_address, ";

        ctes = ctes.concat(this.composeCommonTableExpression(query));
        console.log(ctes);
        let commonTableExpressions = "", joins = " ";
        query.select = "SELECT DISTINCT d.id, d.type, " + specificFields + "d.metadata FROM " + query.table + " d";
