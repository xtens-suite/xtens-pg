...
class PgJSONQueryStrategy extends PgQueryStrategy {
    ...
    composeCommonTableExpression(query, leafSearch, ctes, parentAlias, parentTable) {
        if (!ctes) {
            ...
            let joinClause =  'INNER JOIN ' + joinInfo.name +  ' AS ' + joinInfo.alias + ' ON ' + joinInfo.alias + '."' + joinInfo.parentColumn + '" = ' + query.alias + '.id';
            joinClause += ' INNER JOIN ' + parentAlias + ' ON ' + joinInfo.alias + '."' + joinInfo.childColumn + '" = ' + parentAlias + '.id'
