...
class PgJSONQueryStrategy extends PgQueryStrategy {
  ...
  composeCommonTableExpression(query, leafSearch, ctes, parentAlias, parentTable) {
    ...
    if (!ctes) {
      ...
      ctes.push({
        ...
        filesToBeAggregate: leafSearch && qLen > 0 ? true : false,
      });
    }
  ...
  formatSelectAndGroupFields(infoArray, isLeafSearch) {
    ...
    if (aggrFields.length > 0) {
      formattedFields = 'array_agg( json_build_object(' + aggrFields.join(", ") + ')), '
    }
