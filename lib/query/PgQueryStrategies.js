...
new Map([
    ["data_data", {name: "data_childrendata__data_parentdata", childColumn: "data_parentData", parentColumn: "data_childrenData", alias: "dtdt"}],
    ["data_sample", {name: "id, biobank_code, sample_parentSubject, sample_parentSample", childColumn: "data_parentSample", parentColumn: "sample_childrenData", alias: "dtsm"}],
    ["data_subject", {name: "id, data_parentSubject, data_parentSample, data_parentData", childColumn: "data_parentSubject", parentColumn: "subject_childrenData", alias: "dtsb"}],
    ["sample_sample", {name: "sample_parentsample__sample_childrensample", childColumn: "sample_parentSample", parentColumn: "sample_childrenSample", alias: "smsm"}],
    ["sample_subject", {name: "sample_parentsubject__subject_childrensample", childColumn: "sample_parentSubject", parentColumn: "subject_childrenSample", alias: "smsb"}]
]);
...
class PgJSONQueryStrategy extends PgQueryStrategy {
     ...
     */
    composeCommonTableExpression(query, leafSearch, ctes, parentAlias, parentTable) {
        if (!ctes) {
            ctes = [];
        }
            ...
            let joinClause =  'INNER JOIN ' + joinInfo.name +  ' AS ' + joinInfo.alias + ' ON ' + joinInfo.alias + '."' + joinInfo.childColumn + '" = ' + query.alias + '.id';
            joinClause += ' INNER JOIN ' + parentAlias + ' ON ' + joinInfo.alias + '."' + joinInfo.parentColumn + '" = ' + parentAlias + '.id'

            ctes.push({
                alias: query.alias,
                commonTableExpression: query.alias + " AS (" + query.select + " " + query.where + ")",
                joinClause: joinClause,
                getMetadata: leafSearch && query.getMetadata ? true : false,
        ...
        for (let i=0; i<qLen; i++) {
            ctes = this.composeCommonTableExpression(query.subqueries[i], leafSearch, ctes, alias, query.table);
        }
        return ctes;
    }
    ...
    compose(criteria) {
        ...
        if (criteria.model !== DataTypeClasses.SUBJECT && criteria.wantsSubject) {
          ...
          let firstJoin = 'LEFT JOIN ' + joinInfo.name +  ' AS ' + joinInfo.alias + ' ON ' + joinInfo.alias + '."' + joinInfo.parentColumn + '" = ' + 's.id';
          let secondJoin = 'LEFT JOIN s ON s.id = ' + joinInfo.alias + '."' + joinInfo.parentColumn + '"';
            ctes.push({
                alias: 's',
        ...
        ctes = ctes.concat(this.composeCommonTableExpression(query, criteria.leafSearch));
        if (criteria.leafSearch && ctes.length > 0) {
          for (var i = 0; i < ctes.length; i++) {
            if (ctes[i].label) {
              specificFields += ctes[i].alias + ".id AS " + ctes[i].label + "_id, ";
              if (ctes[i].getMetadata) {
                specificFields += ctes[i].alias + ".metadata AS " + ctes[i].label + ", ";
              }
            }
          }
        }
        ...
        query.select = tempSelect + "d.id, d.type, d.owner, " + specificFields + "d.metadata FROM " + query.table + " d";
