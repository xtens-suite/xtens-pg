...
let fieldsForSubqueriesMap = new Map([
    ...
    [DataTypeClasses.SAMPLE, "id, biobank_code, sample_parentSubject, sample_parentSample"],
    [DataTypeClasses.DATA, "id, data_parentSubject, data_parentSample, data_parentData"],
...
class PgJSONQueryStrategy extends PgQueryStrategy {
    ...
    composeCommonTableExpression(query, leafSearch, ctes, parentAlias, parentTable) {
        if (!ctes) {
            ...
            let joinClause =  "INNER JOIN " + joinInfo.name +  " AS " + joinInfo.alias + " ON " + joinInfo.alias + "." + joinInfo.parentColumn + " = " + parentAlias + ".id";
            ctes.push({
                alias: query.alias,
        ...
        if (criteria.model !== DataTypeClasses.SUBJECT && criteria.wantsSubject) {
          let joinInfo = joinPoolingTables.get(query.table + '_subject' );
            ctes.push({
                ...
                joinClause: "LEFT JOIN " + joinInfo.name +  " AS " + joinInfo.alias + " ON " + joinInfo.alias + "." + joinInfo.parentColumn + " = " + "s.id"
            });
